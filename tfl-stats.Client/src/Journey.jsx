                            ...
                            <table border="1">
                                ...
                                <tbody>
                                    {journey.legs.map((leg, legIndex) => (
                                        <tr key={legIndex}>
                                            <td>{leg.departurePoint?.commonName || 'Unknown'}</td>
                                            <td>{leg.arrivalPoint?.commonName || 'Unknown'}</td>
                                            <td>{leg.mode?.name || 'N/A'}</td>
                                            <td>{leg.departureTime ? new Date(leg.departureTime).toLocaleTimeString() : 'N/A'}</td>
                                            <td>{leg.arrivalTime ? new Date(leg.arrivalTime).toLocaleTimeString() : 'N/A'}</td>
                                            <td>{leg.duration || 'N/A'}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
